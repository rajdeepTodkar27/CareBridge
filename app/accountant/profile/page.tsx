...
import { Save, LogOut, User, CreditCard, LayoutDashboard } from 'lucide-react';
...
interface AccountantProfileForm {
  fullName: string;
  mobileNo: number;
  gender: string;
...
const AccountantProfilePage = () => {
  const { register, handleSubmit, setValue,control, formState: { errors } } = useForm<AccountantProfileForm>();
  ...
  useEffect(() => {
    ...
    const onSubmit: SubmitHandler<AccountantProfileForm> = async (formData) => {
    try {
      setErrMessage("");
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        <div className="bg-white shadow-lg rounded-xl p-8">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Accountant Profile</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card title="Payment History" icon={<CreditCard className="w-6 h-6 text-green-600" />} onClick={() => router.push('/accountant/payment-history')} />
          <Card title="Dashboard" icon={<LayoutDashboard className="w-6 h-6 text-green-600" />} onClick={() => router.push('/accountant/dashboard')} />
        </div>
      </div>
    </main>
  );
