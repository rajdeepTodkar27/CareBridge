...
import { LogOut, Save, User, Stethoscope, CreditCard } from 'lucide-react';

interface PharmasistFormInputs {
  ownerName: string;
  licenseNo: string;
...
const PharmacistProfilePage = () => {
  const { register, handleSubmit, setValue, control, formState: { errors } } = useForm<PharmasistFormInputs>();
  ...
  useEffect(() => {
    axios.get('/api/pharmasist/profile')
      .then(res => {
        ...
        setValue("store", data.store?.name || "");  // assuming populated store
        setImageSrc(data.avtarImg);
        setOldImageSrc(data.avtarImg);
      })
      .catch(() => setErrMessage("Failed to fetch pharmacist profile."));
  }, [setValue]);

  const onSubmit: SubmitHandler<PharmasistFormInputs> = async (formData) => {
    try {
      setErrMessage("");
      ...
      const res = await axios.post('/api/pharmasist/profile', payload);
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        <div className="bg-white shadow-lg rounded-xl p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 mt-6">
            ...
            />
            <Section title="Pharmacist Information">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                <Input label="Owner Name" name="ownerName" register={register} errors={errors} required />
                <Input label="License No" name="licenseNo" register={register} errors={errors} required />
                <Input label="License Authority" name="licenseAuthority" register={register} errors={errors} required />
                <Input label="Education" name="education" register={register} errors={errors} required />
                <Input label="Experience (Years)" name="experience" type="number" register={register} errors={errors} required />
                <Input label="Store Name" name="store" register={register} errors={errors} readOnly />
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <button type="submit" className="flex items-center justify-center gap-2 px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg">
                <Save className="w-5 h-5" /> Save Changes
              </button>
              <button type="button" onClick={handleLogout} className="flex items-center justify-center gap-2 px-6 py-2 border-2 border-red-600 text-red-600 rounded-lg">
