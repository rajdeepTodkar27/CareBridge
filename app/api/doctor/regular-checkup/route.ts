...
export async function GET() {
    try {
        ...
        const regularcheckup = await RegularCheckup.find({ isDone: false })
            .populate({ path: "appointmentRequest", match: { doctor: profile._id }, populate: { path: "patient", select: "patient fullName gender mobileNo" }, select: "description scheduledTime" })
