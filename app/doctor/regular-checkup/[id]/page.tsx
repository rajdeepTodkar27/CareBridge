...
export default function CheckupDetails() {
    ...
    useEffect(() => {
        axios.get(`/api/doctor/regular-checkup/${id}`)
    ...
    const handlemedhis = ()=>{
        router.push(`/doctor/medical-history/${patient.patientUId}`)
    }
            ...
            <main className="px-4 sm:px-8 md:px-10 lg:px-40 py-6 flex justify-center">
                <div className="w-full max-w-5xl">
                    ...
                    <div className="flex justify-between gap-8">
                        ...
                        <Button>Missed Medication</Button>
                    </div>
                    <div className="mb-10">
                        <VitalsTable initialVitals={patient.vitals} patientUid={patient.patientUId} />
...
function InfoGrid({ patient }: { patient?: Patient }) {
    const fields: [string, string | undefined][] = patient
        ? [
...
function CheckupTable({ regularCheckup }: { regularCheckup: Checkup }) {
    const [editMode, setEditMode] = useState(false);
    const [followUpDate, setFollowUpDate] = useState(regularCheckup.followUpDate || "");
    const [notes, setNotes] = useState(regularCheckup.notes || "");
    ...
    const handleSave = async () => {
        try {
            ...
            await axios.put("/api/doctor/regular-checkup/update-checkup", {
                appointmentRequest: par.id,
                followUpDate,
                notes,
            });
            setEditMode(false);
            alert("Follow-up details updated.");
        } catch (err) {
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            <tr className="border-t border-[#dce1e5]">
                                <td className="px-4 py-4 text-[#637988]">
                                    {editMode ? (
                                        <Input
                                            type="date"
                                            value={followUpDate?.substring(0, 10)}
